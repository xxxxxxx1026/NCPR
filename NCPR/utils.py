...
def load_feature_similarity_dict(dataset):
    filename = TMP_DIR[dataset] + '/feature_similarity.json'
    with open(filename, 'r') as f:
        feature_similarity_dict = json.load(f)
...
def load_rl_agent(dataset, filename, epoch_user, hyper, method):
    t = method + '-' + str(hyper)
    model_file = f"{TMP_DIR[dataset]}/RL-agent/{t}/{filename}-epoch-{epoch_user}.pkl"
    # model_file = TMP_DIR[dataset] + '/RL-agent/' + '/{}/' + filename + '-epoch-{}.pkl'.format(t, epoch_user)
    model_dict = torch.load(model_file)
    print('RL policy model load at {}'.format(model_file))
    return model_dict

def save_rl_agent(dataset, model, filename, epoch_user, hyper, method):
    t = method + '-' + str(hyper)
    model_file = f"{TMP_DIR[dataset]}/RL-agent/{t}/{filename}-epoch-{epoch_user}.pkl"
    # model_file = TMP_DIR[dataset] + '/RL-agent/' + '/{}/' + filename + '-epoch-{}.pkl'.format(t, epoch_user)
    if not os.path.isdir(TMP_DIR[dataset] + '/RL-agent/' + t):
        os.makedirs(TMP_DIR[dataset] + '/RL-agent/' + t)
    torch.save(model.state_dict(), model_file)
...
def save_rl_mtric(dataset, filename, epoch, SR, spend_time, hyper, method, mode='train'):
    t = method + '-' + str(hyper)
    PATH = f"{TMP_DIR[dataset]}/RL-log-merge/{t}/{filename}.txt"
    # PATH = TMP_DIR[dataset] + '/RL-log-merge/' + filename + '.txt'
    if mode == 'train':
        if not os.path.isdir(TMP_DIR[dataset] + '/RL-log-merge/' + t):
            os.makedirs(TMP_DIR[dataset] + '/RL-log-merge/' + t)
        with open(PATH, 'a') as f:
            f.write('===========Train===============\n')
        ...
        PATH = f"{TMP_DIR[dataset]}/RL-log-merge/Validation/{t}/{filename}.txt"
        if not os.path.isdir(TMP_DIR[dataset] + '/RL-log-merge/Validation/' + t):
            os.makedirs(TMP_DIR[dataset] + '/RL-log-merge/Validation/' + t)
        with open(PATH, 'a') as f:
            f.write('===========Test===============\n')
            f.write('Testing {} user tuples\n'.format(epoch))
            f.write('Testing SR@5: {}\n'.format(SR[0]))
...
def save_rl_model_log(dataset, filename, epoch, epoch_loss, train_len, hyper, method):
    t = method + '-' + str(hyper)
    PATH = f"{TMP_DIR[dataset]}/RL-log-merge/{t}/{filename}.txt"
    # PATH = TMP_DIR[dataset] + '/RL-log-merge/' + filename + '.txt'
    if not os.path.isdir(TMP_DIR[dataset] + '/RL-log-merge/' + t):
