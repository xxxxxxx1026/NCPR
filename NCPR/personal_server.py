...
class producer(Thread):
    ...
    def _append_queue(self, name, model,addr):
        tmp_q = Queue()
        self.q_list.append(tmp_q)
        tmp_t = consumer(tmp_q, name, model, addr, self.server_socket, args=self.args)
        tmp_t.start()

    def _welcome(self, addr, name):
        x = f"欢迎你, {name}! 我是推荐机器人，请问你喜欢什么类型的音乐？"
    ...
    def run(self):
        while True:
            ...
            name = raw_json['name']
            if raw_json['type'] == 1:
                if name.isdigit() and int(name) >= 0 and int(name) <= 1800:
                    pass
                else:
                    self._welcome(addr, raw_json['name'])
                self._append_queue(raw_json['name'], raw_json['model'], addr)
...
class consumer(Thread):
    def __init__(self, q, name, model, addr, server_socket, args):
        Thread.__init__(self)
        self.q = q
        self.name = name
        self.server_socket = server_socket
    ...
    def _load_model(self, name):
        ...
        if name == "model3":
            print("选择了模型3")
            from RL.SCPR.env_load_user import LoadUserInfoEnv
            from RL.SCPR.env_dialogue import DialogueEnv
            from RL.SCPR.RL_model import Agent, ReplayMemory
        else:
            print("选择了我的模型")
            from RL.env_load_user import LoadUserInfoEnv
            from RL.env_dialogue import DialogueEnv
            from RL_model import Agent,ReplayMemory
        kg = load_kg(self.args.data_name)
        feature_length = len(kg.G['feature'].keys())
        args.attr_num = feature_length
        dataset = load_dataset(self.args.data_name)
        ...
        self.agent = Agent(device=args.device, memory=memory, state_space=state_space, hidden_size=args.hidden,
                           action_space=action_space)
        if args.load_rl_epoch != 0:
            print('Staring loading rl model in epoch {}'.format(args.load_rl_epoch))
            self.agent.load_model(data_name=args.data_name, filename=filename, epoch_user=args.load_rl_epoch, hyper=0.3, method='jaccard')

    def _ask_attribute(self, attr_list):
        x = self.emotional_msg + '那你喜欢' + str(attr_list) + '吗？'
        msg = {
            'name':self.name,
            'msg':x
        }
    ...
    def _recommend_items(self, item_list):
        x = self.emotional_msg + '我猜测你可能会喜欢以下物品:' + str(item_list)
        msg = {
            'name':self.name,
            'msg':x,
        }
    ...
    def _interaction(self):
        while True:
            ...
            if raw_json['type'] == 0:
                if self.reset_flag:
                    self.state = self.env.reset(self.env.name2tag[raw_json['message']])
                    ...
                    if emotional_flag == 1:
                        ...
                        tmp_flag = 'no'
                        self.emotional_msg = "我很抱歉, "
                    else:
                        tmp_flag = 'yes'
                        self.emotional_msg = "太好了！ "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='test', help='the mode in [train, test]')
    ...
    parser.add_argument('--load_rl_epoch', type=int, default=36000, help='the epoch of loading RL model')
