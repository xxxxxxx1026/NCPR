...
from FPAN_yelp.recsysconfig import recsysconfig
from FPAN_yelp.recmodel import MyRec

class RFAPNEnv(object):
    def __init__(self, kg, dataset, data_name, seed=1, max_turn=15, cand_len_size=20, attr_num=20, mode='train', command=6, ask_num=1, entropy_way='weight entropy', fm_epoch=0, method='none', hyper=0.3):
        ...
        self.load_embeds()
        self.load_model("iter13-2020-06-08", False)
        ...
        self.large_embed2 = np.zeros((self.feature_length, 64), dtype=np.float)
        self._calu_large_embed()
        self._calu_large_embed2()
    ...
    def load_model(self, extra_name=None, transfer_to_cpu=False):
        ...
        if transfer_to_cpu:
            pretrained_dict = torch.load("/".join([self.rec_model_path, name]), map_location='cpu')
        else:
            pretrained_dict = torch.load("/".join([self.rec_model_path, name]), map_location='cpu')
        model_dict = self.rec.state_dict()
        x_list = ['gu_linear.weight', 'gu_linear.bias', 'gni_linear.weight', 'gni_linear.bias']
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in x_list}
        print(pretrained_dict.keys())
        print(self.rec.attribute_embed.weight.shape)
        model_dict.update(pretrained_dict)
        self.rec.load_state_dict(model_dict)

    def load_embeds(self):
        print("heeeeeeeeeeeeeeeeeeeeeeeeeeeee")
        self.rec.graph_prop()
        self.ui_embed = torch.cat((self.rec.user_embed.weight.data, self.rec.item_embed.weight.data), 0)
        self.user_embed = self.rec.user_embed.weight.data.cpu().numpy()
        self.item_embed = self.rec.item_embed.weight.data.cpu().numpy()
        self.feature_emb = self.rec.attribute_embed.weight.data.cpu().numpy()
        self.ui_embeds = self.ui_embed.data.cpu().numpy()
        # if self.rec.gpu:
        #     self.ui_embeds.cuda()
        #     self.user_embed.cuda()
        #     self.item_embed.cuda()
        #     self.feature_emb.cuda()
        print(self.user_embed.shape)
        print(self.item_embed.shape)
        print(self.feature_emb.shape)
        print(self.ui_embed.shape)

    # def get_current_user_embedding(self):
    #     # pos_att = self.feature_emb[self.acc_samll_fea, :]
    #     # neg_att = self.feature_emb[self.rej_samll_fea, :]
    #     pos_att = self.large_embed2[self.user_acc_feature, :]
    #     neg_att = self.large_embed2[self.user_rej_feature, :]
    #     neg_item = [x + self.user_length for x in self.user_rej_items]
    #     neg_item = self.ui_embeds[neg_item, :]
    #     avg_pos_att = np.sum(pos_att, axis=0) / len(self.user_acc_feature)
    #     avg_neg_att = np.sum(neg_att, axis=0) / len(self.user_rej_feature)
    #     avg_neg_item = np.sum(neg_item, axis=0) / len(self.user_rej_items)
    #     print(avg_neg_att + avg_pos_att)
    #     self.user_embed = self.user_embed - avg_neg_att + avg_pos_att
    ...
    def update_user_embedding(self):
        ...
        neg_item_list = []
        #
        # for _ in self.user_acc_feature:
        #     pos_att_list.append(_)
        pos_att_mask = [True] * len(self.acc_samll_fea)

        # for _ in self.user_rej_feature:
        #     neg_att_list.append(_)
        if len(self.user_rej_feature) == 0:
            neg_att_list = [0]
            neg_att_mask = [False]
        else:
            neg_att_mask = [True] * len(self.rej_samll_fea)

        # for _ in self.user_rej_items:
        #     neg_item_list.append(_)
        if len(self.user_rej_items) == 0:
            neg_item_list = [0]
            neg_item_mask = [False]
        else:
            neg_item_mask = [True] * len(self.user_rej_items)

        user = torch.tensor(user)
        pos_att_list = torch.tensor(self.acc_samll_fea)
        pos_att_mask = torch.tensor(pos_att_mask)
        neg_att_list = torch.tensor(self.rej_samll_fea)
        neg_att_mask = torch.tensor(neg_att_mask)
        neg_item_list = torch.tensor(self.user_rej_items)
        neg_item_mask = torch.tensor(neg_item_mask)

        if self.rec.gpu:
            user = user.cuda()
            pos_att_list = pos_att_list.cuda().type(torch.long)
            pos_att_mask = pos_att_mask.cuda()
            neg_att_list = neg_att_list.cuda().type(torch.long)
            neg_att_mask = neg_att_mask.cuda()
            neg_item_list = neg_item_list.cuda().type(torch.long)
            neg_item_mask = neg_item_mask.cuda()


        print(self.user_embed)
        self.user_embed = self.rec.get_current_user_embedding(user, pos_att_list, pos_att_mask, \
                                            neg_att_list, neg_att_mask, neg_item_list, neg_item_mask)
        self.user_embed = self.user_embed.data.detach().cpu().numpy()
        print(self.user_embed)
    ...
    def step(self, action):   #action:0  ask   action:1  recommend   setp=MAX_TURN  done
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            # self.get_current_user_embedding()
    ...
    def _calu_large_embed2(self):
        for i in range(self.attr_state_num):
            small_ids = self.kg.G['large_feature'][i]['link_to_feature']
            small_embed = np.sum(self.feature_emb[small_ids, :], axis=0)
            self.large_embed2[i] = small_embed / len(small_ids)
