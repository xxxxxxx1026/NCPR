...
class RFAPNEnv(object):
    def __init__(self, kg, dataset, data_name, seed=1, max_turn=15, cand_len_size=20, attr_num=20, mode='train', command=6, ask_num=1, entropy_way='weight entropy', fm_epoch=0, method='none', hyper=0.3):
        ...
        # # self.load_model("iter13-2021-05-04", False)
    ...
    def get_current_user_embedding(self):
        ...
        avg_pos_att = np.sum(pos_att, axis=0) / (len(self.user_acc_feature) + self.eps)
        avg_neg_att = np.sum(neg_att, axis=0) / (len(self.user_rej_feature) + self.eps)
        avg_neg_item = np.sum(neg_item, axis=0) / (len(self.user_rej_items) + self.eps)
        self.user_embed = self.user_embed + 0.1 * (avg_pos_att - avg_neg_att - avg_neg_item)
        print(self.user_embed)
