...
# from RL.env_fpan import FPANEnv
...
def dqn_valid(args, kg, dataset, agent, filename, i_episode):
    test_env = EnvDict[args.data_name](kg, dataset, args.data_name, seed=args.seed, max_turn=args.max_turn,
                                       cand_len_size=args.cand_len_size, attr_num=args.attr_num, mode='valid',
                                       command=args.command, ask_num=args.ask_num, entropy_way=args.entropy_method,
                                       fm_epoch=args.fm_epoch, hyper=args.hyper, method=args.method)
    ...
    test_filename = 'Validation-epoch-{}-'.format(i_episode) + filename
    ...
    for user_num in range(user_size+1):  #user_size
        ...
        if user_num % args.observe_num == 0 and user_num > 0:
            ...
            print('Total validation epoch_uesr:{}'.format(user_num + 1))
    ...
    save_rl_mtric(dataset=args.data_name, filename=filename, epoch=user_num, SR=SR_all, spend_time=time.time() - start,
                  mode='valid', hyper=args.hyper, method=args.method)
    save_rl_mtric(dataset=args.data_name, filename=test_filename, epoch=user_num, SR=SR_all, spend_time=time.time() - start,
                  mode='valid', hyper=args.hyper, method=args.method)  # save RL SR
    print('save valid evaluate successfully!')
    ...
    PATH = f"{TMP_DIR[args.data_name]}/RL-log-merge/Validation/{t}/{test_filename}.txt"
    if not os.path.isdir(TMP_DIR[args.data_name] + '/RL-log-merge/Validation/' + t):
        os.makedirs(TMP_DIR[args.data_name] + '/RL-log-merge/Validation/' + t)
    # PATH = TMP_DIR[args.data_name] + '/RL-log-merge/' + test_filename + '.txt'
    with open(PATH, 'a') as f:
        f.write('Training epocch:{}\n'.format(i_episode))
        f.write('===========Test Turn===============\n')
        f.write('Validation {} user tuples\n'.format(user_num))
        for i in range(len(SRturn_all)):
            f.write('Validation SR-turn@{}: {}\n'.format(i, SRturn_all[i]))
        f.write('================================\n')
